...
describe("Authentication", () => {
  ...
  test("User is able to signin and token is working", async () => {
    const username = "Bob" + Math.random();
    const password = "qwerty123";
    const type = "admin";
    await axios.get(`${SERVER_URL}/api/v1/signup`, {
      username,
      password,
      type,
    });
  ...
  test("User is not able to signin cause wrong username and password", async () => {
    const username = "Bob" + Math.random();
    const password = "qwerty123";
    const type = "admin";
    await axios.get(`${SERVER_URL}/api/v1/signup`, {
...
describe("element creation", () => {
  ...
  test("user cant make to admin endpoints", async () => {
    ...
    expect(elementRes.statusCode).toBe(404);
    expect(mapRes.statusCode).toBe(404);
    expect(avatarRes.statusCode).toBe(404);
    expect(updateElementRes.statusCode).toBe(404);
  });
